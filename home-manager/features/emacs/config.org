...
#+BEGIN_SRC emacs-lisp
  (defun oposs/random-date (&optional start-date end-date)
    (defun date-to-int (str)
      (time-convert (date-to-time str) 'integer))
    (let ((start-date (date-to-int (or start-date "1600-01-01")))
          (end-date (date-to-int (or end-date "2300-01-01"))))
  ...
  (defun oposs/doomsday-random ()
    (interactive)
    (setq random-date (oposs/random-date))
    (if (string=
         (format-time-string "%A" random-date)
         (nth 1 (read-multiple-choice
                 (format "What day did %s fall ?" (format-time-string "%B %d, %Y" random-date))
  ...
  (defalias 'oposs/play-game 'oposs/doomsday-random)
